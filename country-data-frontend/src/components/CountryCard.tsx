...
export const CountryCard: React.FC<{ country: Country | CountryDetails }> = ({ country }) => {
   const isCountryDetails = (country: Country | CountryDetails): country is CountryDetails => {
      return (country as CountryDetails).name?.common !== undefined;
   };

   return (
      <div className="bg-white rounded-lg shadow-md p-4 flex items-center space-x-4 md:space-x-6">
         {country.flag ? (
            isCountryDetails(country) ? (
         ...
         <div className="flex flex-col">
            <h2 className="font-semibold text-base md:text-lg">
               {isCountryDetails(country) ? country.name.common : country.name}
            </h2>
